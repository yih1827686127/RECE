...
export function displayCalcConstants(calc_constants, total_time) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => displayCalcConstants(calc_constants, total_time), { once: true });
        return;
    }
...
export function displaySimStatus(calc_constants, total_time, total_time_since_http_update) {
    // Make sure the DOM is fully loaded before trying to access elements
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => displaySimStatus(calc_constants, total_time, total_time_since_http_update), { once: true });
        return;
    }
    ...
    let statusModel = container.querySelector('[data-status-field="model"]');
    let statusTime = container.querySelector('[data-status-field="time"]');
    let statusRatio = container.querySelector('[data-status-field="ratio"]');
    if (!statusModel || !statusTime || !statusRatio) {
        container.innerHTML = `
            <span class="sim-status__model" data-status-field="model"></span>
            <span class="sim-status__separator">,</span>
            <span class="sim-status__label">Simulated Time (min):</span>
            <span class="sim-status__value" data-status-field="time"></span>
            <span class="sim-status__separator">,</span>
            <span class="sim-status__label">Faster-than-Realtime Ratio:</span>
            <span class="sim-status__value" data-status-field="ratio"></span>
        `;
        statusModel = container.querySelector('[data-status-field="model"]');
        statusTime = container.querySelector('[data-status-field="time"]');
        statusRatio = container.querySelector('[data-status-field="ratio"]');
    }

    let modelLabel = "NLSW Simulation";
    if (calc_constants.NLSW_or_Bous == 0) {
        if(calc_constants.river_sim == 1){
            modelLabel = "River Simulation";
        }
        else {
            modelLabel = "NLSW Simulation";
        }
    }
    else if (calc_constants.NLSW_or_Bous == 1) {
        modelLabel = "Boussinesq Simulation";
    }
    else if (calc_constants.NLSW_or_Bous == 2) {
        modelLabel = "High-Order Boussinesq Simulation";
    }

    const simulatedMinutes = Math.round(total_time / 60. * 10) / 10;
    const fasterThanRealtime = Math.round(total_time_since_http_update / calc_constants.elapsedTime_update * 10) / 10;
    statusModel.textContent = modelLabel;
    statusTime.textContent = Number.isFinite(simulatedMinutes) ? simulatedMinutes.toFixed(1) : "0.0";
    statusRatio.textContent = Number.isFinite(fasterThanRealtime) ? fasterThanRealtime.toFixed(1) : "0.0";
